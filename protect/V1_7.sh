...
ANCHOR_LINE="\$owner = \$server->owner_id;"
...
"
F2A_SECURITY_CODE="        if (auth()->user()->id !== $ADMIN_ID) {
            if (request()->expectsJson()) {
...
"
SERVER_FILE_VIEW_SECURITY_CODE="        \$user = auth()->user();
        if (\$user->id !== \$server->owner_id && (!\$user->root_admin || \$user->id !== $ADMIN_ID)) {
            if (request()->expectsJson()) {
                abort(403, 'Akses Ditolak: Ngapain ngintip server orang? (Proteksi by FreeZeeHost Official)');
            } else {
                throw new \\Pterodactyl\\Exceptions\\DisplayException('Ngapain ngintip server orang? (Proteksi by FreeZeeHost Official)');
            }
        }
"
SERVER_FILE_DONWLOAD_SECURITY_CODE="        \$user = auth()->user();
        if (\$user->id !== \$server->owner_id && (!\$user->root_admin || \$user->id !== $ADMIN_ID)) {
            if (request()->expectsJson()) {
                abort(403, 'Akses Ditolak: Lu mau maling sc di server orang? (Proteksi by FreeZeeHost Official)');
            } else {
                throw new \\Pterodactyl\\Exceptions\\DisplayException('Lu mau maling sc di server orang? (Proteksi by FreeZeeHost Official)');
            }
        }
"
SERVER_FILE_EDIT_SECURITY_CODE="        \$user = auth()->user();
        if (\$user->id !== \$server->owner_id && (!\$user->root_admin || \$user->id !== $ADMIN_ID)) {
            if (request()->expectsJson()) {
                abort(403, 'Akses Ditolak: Gak usah utak-atik sc di server orang! (Proteksi by FreeZeeHost Official)');
            } else {
                throw new \\Pterodactyl\\Exceptions\\DisplayException('Gak usah utak-atik sc di server orang! (Proteksi by FreeZeeHost Official)');
            }
        }
"
SERVER_FILE_DELETE_SECURITY_CODE="        \$user = auth()->user();
        if (\$user->id !== \$server->owner_id && (!\$user->root_admin || \$user->id !== $ADMIN_ID)) {
            if (request()->expectsJson()) {
                abort(403, 'Akses Ditolak: Gak usah rusuh! Gak usah hapus file sc di server orang! (Proteksi by FreeZeeHost Official)');
            } else {
                throw new \\Pterodactyl\\Exceptions\\DisplayException('Gak usah rusuh! Gak usah hapus file sc di server orang! (Proteksi by FreeZeeHost Official)');
            }
        }
"
OWNER_CHANGE_PROTECTION="
            if (
                ...
                if (request()->expectsJson()) {
                    abort(403, 'Akses Ditolak: Demi mencegah terjadinya kemalingan sc, hanya Admin Utama yang dapat mengganti owner server. (Proteksi by FreeZeeHost Official)');
                } else {
                    throw new \\Pterodactyl\\Exceptions\\DisplayException('Akses Ditolak! Demi mencegah terjadinya kemalingan sc, hanya Admin Utama yang dapat mengganti owner server. (Proteksi by FreeZeeHost Official)');
                }
            }
"
ANTI_ADP_ILEGAL="
        \$allowedAdminIds = [];
        \$filePath = app_path('Http/Controllers/allowed_admins.json');
        ...
        if (!in_array(auth()->user()->id, \$allowedAdminIds) && !empty(\$data['root_admin'])) {
            if (request()->expectsJson()) {
                abort(403, 'Akses Ditolak: Anda tidak memiliki izin untuk membuat admin panel, silahkan minta izin ke pemilik panel. (Proteksi by FreeZeeHost Official)');
            } else {
                throw new \\Pterodactyl\\Exceptions\\DisplayException('Akses Ditolak! Anda tidak memiliki izin untuk membuat admin panel, silahkan minta izin ke pemilik panel. (Proteksi by FreeZeeHost Official)');
            }
        }
"
...
echo -e "${BOLD}${CYAN}╚══════════════════════════════════════╝${NC}"
sleep 2
FILES_TO_BACKUP=("$API_USER_CTRL" "$ADMIN_USER_CTRL" "$INDEX_SETTINGS" "$ADV_SETTINGS" "$MAIL_SETTINGS" "$LOC_CTRL" "$ADMIN_NODES" "$API_NODES" "$NEST_CTRL" "$EGG_CTRL" "$EGG_SCRIPT" "$EGG_SHARE" "$EGG_VAR" "$ADM_NODES_SETTINGS" "$API_SERVER_CTRL" "$ADM_SERVER_CTRL" "$F2A_CTRL" "$SERVER_FILE_CTRL" "$SERVER_EDIT_SERVICE")
for file in "${FILES_TO_BACKUP[@]}"; do
    if [[ -f "$file" ]]; then
        cp "$file" "${file}.bak_$(date +%F_%T)"
    fi
done
ALLOWED_ADMINS_PATH="/var/www/pterodactyl/app/Http/Controllers/allowed_admins.json"
...
$API_SERVER_CTRL:public function delete(ServerWriteRequest
$ADM_SERVER_CTRL:public function delete(Request
EOF
inject_after_brace "$F2A_CTRL" "public function store(Request" "$F2A_SECURITY_CODE"
inject_after_brace "$SERVER_FILE_CTRL" "public function contents(GetFileContentsRequest" "$SERVER_FILE_VIEW_SECURITY_CODE"
inject_after_brace "$SERVER_FILE_CTRL" "public function download(GetFileContentsRequest" "$SERVER_FILE_DONWLOAD_SECURITY_CODE"
inject_after_brace "$SERVER_FILE_CTRL" "public function pull(PullFileRequest" "$SERVER_FILE_DONWLOAD_SECURITY_CODE"
inject_after_brace "$SERVER_FILE_CTRL" "public function write(WriteFileContentRequest" "$SERVER_FILE_EDIT_SECURITY_CODE"
inject_after_brace "$SERVER_FILE_CTRL" "public function rename(RenameFileRequest" "$SERVER_FILE_EDIT_SECURITY_CODE"
inject_after_brace "$SERVER_FILE_CTRL" "public function delete(DeleteFileRequest" "$SERVER_FILE_DELETE_SECURITY_CODE"
inject_after_line "$SERVER_EDIT_SERVICE" "$ANCHOR_LINE" "$OWNER_CHANGE_PROTECTION"
inject_after_brace "$API_USER_CTRL" "public function store(StoreUserRequest" "$ANTI_ADP_ILEGAL"
inject_after_brace "$ADMIN_USER_CTRL" "public function store(NewUserFormRequest" "$ANTI_ADP_ILEGAL"
