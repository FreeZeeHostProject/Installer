...
USER_UPDATE_SECURITY="        /**
        ...
        */
        ...
        if (\$currentUser->id != $ADMIN_ID && \$currentUser->id != \$targetUser->id) { 
            if (
...
$API_SERVERMANAGE_CTRL:public function suspend(ServerWriteRequest
$ADM_SERVER_CTRL:public function delete(Request
$ADM_SERVER_CTRL:public function manageSuspension(Request
$ADM_SERVERTF_CTRL:public function transfer(Request
EOF
inject_after_brace "$API_USER_CTRL" "public function update(UpdateUserRequest" "$USER_UPDATE_SECURITY"
inject_after_brace "$ADMIN_USER_CTRL" "public function update(UserFormRequest" "$USER_UPDATE_SECURITY"
inject_after_brace "$BUILD_SERVICE" "public function handle(Server \$server, array \$data): Server" "$SERVER_BUILD_PROTECT"
...
inject_after_brace "$SERVER_FILE_CTRL" "public function delete(DeleteFileRequest" "$SERVER_SECURITY_CODE"
inject_after_brace "$SERVER_FILE_CTRL" "public function decompress(DecompressFilesRequest" "$SERVER_SECURITY_CODE"
inject_after_brace "$SERVER_SETTINGS_CTRL" "public function reinstall(ReinstallServerRequest" "$SERVER_SECURITY_CODE"
inject_after_brace "$API_CLIENT_SERVER_CTRL" "public function index(GetServerRequest" "$SERVER_SECURITY_CODE"
