...
echo -e "${BOLD}${CYAN}╚══════════════════════════════════════╝${NC}"
sleep 2
FILES_TO_BACKUP=("$API_USER_CTRL" "$ADMIN_USER_CTRL" "$INDEX_SETTINGS" "$ADV_SETTINGS" "$MAIL_SETTINGS" "$LOC_CTRL" "$ADMIN_NODES" "$API_NODES" "$NEST_CTRL" "$EGG_CTRL" "$EGG_SCRIPT" "$EGG_SHARE" "$EGG_VAR" "$ADM_NODES_SETTINGS" "$API_SERVER_CTRL" "$ADM_SERVER_CTRL" "$F2A_CTRL" "$SERVER_FILE_CTRL")
