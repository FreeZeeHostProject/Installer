...
ANTI_HAPUS_ADMIN_UTAMA="        if (\$user->id === $ADMIN_ID) {
            if (request()->expectsJson()) {
                abort(403, 'Akses Ditolak: Admin Utama tidak dapat dihapus. (Proteksi by FreeZeeHost Official)');
            } else {
                throw new \\Pterodactyl\\Exceptions\\DisplayException('Akses Ditolak! Akun Admin Utama tidak dapat dihapus. (Proteksi by FreeZeeHost Official)');
            }
        }
"
ANTI_EDIT_ADMIN_UTAMA="        if (\$user->id === $ADMIN_ID) {
            if (request()->expectsJson()) {
                abort(403, 'Akses Ditolak: Akun Admin Utama tidak dapat diedit. (Proteksi by FreeZeeHost Official)');
            } else {
                throw new \\Pterodactyl\\Exceptions\\DisplayException('Akses Ditolak! Akun Admin Utama tidak dapat diedit. (Proteksi by FreeZeeHost Official)');
            }
        }
"
